...
$$\begin{aligned} x\(t\) \cdot e^{-jn\omega_0t} &= \sum_{k=-\infty}^{\infty}{a_ke^{j\omega\_0t\(k-n\)}}\ \int_{T}{x\(t\) \cdot e^{-jn\omega_0t}dt} &= \sum_{k=-\infty}^{\infty}{a_k\int_{T}{e^{j\omega\_0t\(k-n\)}}dt}\ &= \begin{cases} Ta\_k & \text{if } $k=n$,\ 0 & \text{else } \end{cases}\end{aligned}$$
...
{% hint style="info" %}
### Theorem 4 \(Parseval's Theorem\) <a id="theorem-4"></a>
