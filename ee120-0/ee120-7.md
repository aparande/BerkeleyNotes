...
What this tells us is that the Fourier Transform of our sampled signal is a series of copies of $$X(\omega)$$, each centered at $$k\omega_0$$ where $$\omega_0 = \frac{2\pi}{T}$$ For example, lets say that our original signal has the following Fourier Transform. Notice the signal is band-limited by $$\omega_M$$.

![](https://github.com/aparande/BerkeleyNotes/tree/baf2bbb9d79cd7872fefb9525ebaf7bbb6296fcd/.gitbook/assets/f720e3cc7c9c146540093fe305252892ebf80c9f.png)

There are two major cases: if $$\omega_0 > 2\omega_m$$ and $$\omega_0 < 2\omega_M$$. **Case One:** $$\omega_s > 2\omega_m$$

![](https://github.com/aparande/BerkeleyNotes/tree/baf2bbb9d79cd7872fefb9525ebaf7bbb6296fcd/.gitbook/assets/72a019c68af425b148cbc21e7a3709ced6a5572e.png)

When $$\omega_s > 2\omega_M$$, the shifted copies of the original $$X(\omega)$$ \(shown in blue\) do not overlap with each other or which the original copy. If we wanted to recover the original signal, we could simply apply a low pass filter to isolate the unshifted copy of $$X(\omega)$$ and then take the inverse Fourier Transform. **Case Two:** $$\omega_s < 2\omega_m$$

![](https://github.com/aparande/BerkeleyNotes/tree/baf2bbb9d79cd7872fefb9525ebaf7bbb6296fcd/.gitbook/assets/83046672178ba3bb6c8d2d9e6885d19a71ccad6d.png)
...
When $$\omega_0 = 2\omega_M$$, then our ability to reconstruct the original signal depends on the shape of its Fourier Transform. As long as $$X_p(k\omega_m)$$ are equal to $$X(\omega_m)$$ and $$X(-\omega_m$$\), then we can apply an LPF because we can isolate the original $$X(\omega)$$ and take its inverse Fourier Transform.
...
{% hint style="info" %}
### Theorem 18 \(CT Nyquist Theorem\) <a id="theorem-18"></a>
...
{% hint style="info" %}
### Theorem 19 \(DT Nyquist Theorem\) <a id="theorem-19"></a>

Suppose a discrete signal $$x$$ is bandlimited by $$\frac{\pi}{N}$$ and we sample it at a rate of $$\omega_s > 2\omega_M$$, then the signal $$x_r[n]$$ reconstructed by sinc interpolation is exactly $$x[n]$$
{% endhint %}
...
Notice that we have two ways of representing our sample signal. We can either write it as a discrete time signal $$x_d[n] = x(nT)$$ or we can write it as an impulse train $$x_p(t)=\sum_{-\infty}^{\infty}{x(nT)\delta(t-nT)}$$. Based on their Fourier Transforms,
