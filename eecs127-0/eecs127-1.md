...
A norm on the vector space $$\mathcal{X}$$ is a function $$\|\cdot\|:\mathcal{X}\rightarrow\mathbb{R}$$ which satisfies: 1. $$\|\mathbf{x}\|\geq 0$$ with equality if and only if $$\mathbf{x}=\boldsymbol{0}$$

1. $$\|\mathbf{x}+\mathbf{y}\|\leq\|\mathbf{x}\|+\|\mathbf{y}\|$$
2. $$\|\alpha \mathbf{x}\| = |\alpha|\|\mathbf{x}\|$$ for any scalar $$\alpha$$.
{% endhint %}
...
{% hint style="info" %}
### Theorem 1 \(Holder Inequality\) <a id="theorem-1"></a>
...
{% hint style="info" %}
### Theorem 4 \(Fundamental Theorem of Linear Algebra\) <a id="theorem-4"></a>
...
{% hint style="info" %}
### Theorem 5 \(Spectral Theorem\) <a id="theorem-5"></a>
