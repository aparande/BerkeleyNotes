...
[\[defn:plant\]]{#defn:plant label="defn:plant"}

In general, there are two different types of control.

Open-Loop control is where we pass a reference directly to the actuator
to control the plant (see
...
Closed loop control is using the output of our system and comparing it
to the reference in order to generate the control signal (see
...
The control law $K(e)$ is a function of error applied by the controller
to determine the inputs to the plant.
...
Linearization is when a nonlinear system $f(\V{x})$ is approximated by
the first two terms of its Taylor series about a particular operating
point.
$$f(\V{x}_0 + \delta \V{x}) \approx f(\V{x}_0) + \nabla_x|_{\V{x}_0+\delta\V{x}}\delta\V{x}$$
[\[defn:linearization\]]{#defn:linearization label="defn:linearization"}
...
System variables are variables which depend on either the input or the
system's internal state. [\[defn:system-vars\]]{#defn:system-vars
label="defn:system-vars"}

The state variables of a system are the smallest set of linear
independent system variables that can uniquely determine all the other
system variables for all $t > 0$. [\[defn:state-vars\]]{#defn:state-vars
...
The zero-input response is how the system will behave when no input is
supplied. $$\V{x}(t) = e^{At}\V{x}(0)$$
[\[defn:zero-input-response\]]{#defn:zero-input-response
label="defn:zero-input-response"}

The zero-state response is how the system response to an input when its
initial state is $\V{x}(0) = \bs{0}$. It is the convolution of the input
with $e^{At}B\V{u}(t)u(t)$ where $u(t)$ is the unit step.
...
A system is controllable if for any initial state $\V{x}_0$, we can
reach a new state $\V{x}_f$ in finite time with no constraints on the
input $\V{u}$. [\[defn:controllability\]]{#defn:controllability
...
[\[thm:controllability\]]{#thm:controllability
label="thm:controllability"}

### Observability

A system is observable if for any initial state $\V{x}_0$, we can
determine $\V{x}_0$ from $u(t)$ and $y(t)$ over a finite time interval.
[\[defn:observability\]]{#defn:observability label="defn:observability"}
...
[\[thm:observability\]]{#thm:observability label="thm:observability"}
...
The step response of a system is how a system $H(s)$ responds to a step
input. $$y(t) = \mathcal{L}^{-1}\left\{ \frac{H(s)}{s} \right\}$$
[\[defn:step-response\]]{#defn:step-response label="defn:step-response"}
...
The Time to Peak ($T_p$) of a system is how long it takes to reach is
largest value in the step response.
...
The Percent Overshoot ($\% O.S$) of a system is by how much it will
overshoot the step response. [\[defn:percent-os\]]{#defn:percent-os
...
The Settling Time ($T_s$) of a system is how long it takes for the
system to start oscillating within 2% of its final value.
...
$\exists K_u, K_x < \infty$ such that
...
$$\lim_{t\to\infty}\V{x}(t) = \bs{0}.$$ If instead
$\lim_{t\to\infty}\V{x}(t) = \infty$, then the system is unstable.

If all poles are in the left half plane and the number of zeros is less
than or equal to the number of poles, then the system is BIBO stable.
[\[thm:bibo-stability\]]{#thm:bibo-stability label="thm:bibo-stability"}

A system is called marginally stable if the zero-input response does not
converge to $\bs{0}$.
[\[defn:marginal-stability\]]{#defn:marginal-stability
label="defn:marginal-stability"}

A system is marginally stable if there is exactly one pole at $s=0$ or a
pair of poles at $s=\pm j\omega_0$.
...
The final value theorem says that for a function whose unilateral
laplace transform has all poles in the left half plane,
...
The position constant determines how well a system can track a unit
step. $$K_p = \lim_{s\to0}G(s)
...
The acceleration constant determines how well a system can track a
parabola. $$K_a = \lim_{s\to0}s^2G(s)
...
The internal model principle is that if the system in the feedback loop
has a model of the input we want to track, then it can track it exactly.
[\[defn:interal-model\]]{#defn:interal-model label="defn:interal-model"}
...
The frequency response of the system determines how it scales pure
frequencies. It is equivalent to the Laplace transform evaluated on the
imaginary axis.
...
The gain margin $G_m$ is the change in the open loop gain required to
make the closed loop system unstable.
[\[defn:gain-margin\]]{#defn:gain-margin label="defn:gain-margin"}

The phase margin $\phi_m$ is the change in the open loop phase required
...
The root locus is the set of all $s\in\mathbb{C}$ such that $\exists K$
where $\Delta(s) = 0$. [\[defn:root-locus\]]{#defn:root-locus
...
The number of branches in the root locus is equal to the number of
closed loop poles where a branch is the path traveled by a single pole
as $K$ is varied. [\[thm:rl-one\]]{#thm:rl-one label="thm:rl-one"}
...
The real axis segments of the root locus are to the left of an odd
number of open loop poles and zeros. [\[thm:rl-three\]]{#thm:rl-three
label="thm:rl-three"}
...
The root locus begins at the open loop poles and ends at the open loop
zeros [\[eqn:rl-four\]]{#eqn:rl-four label="eqn:rl-four"}
...
Poles beginning at complex locations will depart at an angle $\theta$
where $$\sum_{i=1}^m (p + z_i) - \sum_{i=1}^n (p + p_i) = (2l+1)\pi$$
...
Given a pole location $p$, $$K = \left|\frac{1}{G(p)H(p)}\right|.$$
[\[thm:rl-nine\]]{#thm:rl-nine label="thm:rl-nine"}
...
A Bode plot is a plot of the magnitude and phase of the frequency
response with the magnitude on a log-log scale and the phase on a
semi-log scale. [\[defn:bode-plot\]]{#defn:bode-plot
...
[\[defn:contour\]]{#defn:contour label="defn:contour"}

A mapping is a function that takes a point in the complex plane and
transforms it into another point in the complex plane.
[\[defn:mapping\]]{#defn:mapping label="defn:mapping"}

The Nyquist Criterion says that if $N$ is the number of
counter-clockwise encirclements of zero of a contour mapped by a
...
A proportional integral controller applies an input which is a linear
combination of the scaled and integrated error signal.
$$H(s) = K_p+\frac{K_i}{s} = K_p \frac{s + \frac{K_i}{K_p}}{s}$$
[\[defn:pi-controller\]]{#defn:pi-controller label="defn:pi-controller"}
...
A proportional derivative controller applies an input which is a linear
combination of the scaled and differentiated error signal.
$$H(s) = K_p + K_ds = K_d\left(s + \frac{K_p}{K_d}\right)$$
[\[defn:pd-controller\]]{#defn:pd-controller label="defn:pd-controller"}
...
A proportional integral derivative controller applies an input which is
a linear combination of the scaled, differentated, and integrated error
signals.
...
Suppose we want to control our system to send the state to $\bs{0}$ over
an infinite time horizon using the input $-K\V{x}$. We want to do this
by optimizing a cost function that penalizes control effort and the
...
For a transfer function $G(z)$ which has all poles in the unit circle,
the final value of $g[n]$ is given by
...
Every square matrix $A$ satisfies its own characteristic polynomial if
there are no repeated eigenvalues. $$\Delta(A) = 0$$
[\[thm:cayley-hamilton\]]{#thm:cayley-hamilton
...
$$e^{At} = \sum_{i=0}^{n-1}\alpha_i(t)A^i$$ for some $\alpha_i$ which
