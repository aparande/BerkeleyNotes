...
Using definition 4, we can see that around our operating point, we have
$$f(\mathbf{x}) - f(\mathbf{x}_0) = \delta f(\mathbf{x}) \approx \nabla_x|_{\mathbf{x}_0+\delta\mathbf{x}} \delta\mathbf{x}   \qquad (1)$$
equation 0 will hold so long as $$\delta\mathbf{x}$$ is small enough to
...
matrix $$D$$. Together, equation 1 and equation 2 are the state-space
equations of the system.
$$\begin{aligned}   \frac{d^{}\mathbf{x}}{dt^{}} &= A\mathbf{x} + B\mathbf{u} \qquad (2)\\   \mathbf{y} &= C\mathbf{x} + D \mathbf{u} \label{eqn:stat\qquad (3)$$
...
$$H(s) = \frac{Y(s)}{U(s)} = C(sI-A)^{-1}B+D.   \qquad (4)$$ equation 3
makes it very clear that the poles of the system are the same as the
...
Together, `\cref{eqn:phase-var-1,eqn:phase-var-2}`{=latex} are known as
**Phase Variable Form**. Notice that the characteristic polynomial of
the $$A$$ matrix when it is in phase variable form is
...
the same for state-space equations? equation 1 is a inhomogenous,
...
Series expansion. With this definition, we can solve equation 1 using
integrating factors. If we let $$e^{-At}$$ be our integrating factor,
then multiplying it to both sides of equation 1 gives
...
Notice that equation 6 is broken into two pieces.
...
A theorem analogous to theorem 0 exists for observability.
...
Using equation 7, we see that the derivative is first equal to 0 when
$$t = \frac{\pi}{\omega_d}$$. $$\therefore T_p = \frac{\pi}{\omega_d}$$
...
Recall equation 6 which told us the time-domain solution to state-space
equations was
...
$$\frac{d^{}\mathbf{x}}{dt^{}} = \boldsymbol{0} = A\mathbf{x}_{ss} + B\cdot I \implies \mathbf{x}_{ss} = -A^{-1}B   \qquad (9)$$
...
$$\Delta(s) = D_GD_H+KN_GN_H = 1 + K\frac{N_GN_H}{D_GD_H} = 0   \qquad (10)$$
Clearly, no matter what $$K$$ is, the poles must satisfy two criteria.
$$\left|k\frac{N_GN_H}{D_GD_H}\right| = 1 \qquad \angle K\frac{N_GN_H}{D_GD_H} = (2r+1)\pi,\quad r\in\mathbb{Z}   \qquad (11)$$
...
All points on the root locus must satisfy equation 10.
...
Going back to equation 10, we can alternatively express the angle
criteria as
...
### Theorem 10
  ...
  The root locus intersects the imaginary axis at points where $$     \sum_{i=1}^m \angle (j\omega + z_i) - \sum_{i=1}^n \angle (j\omega + p_i) = (2l+1)\pi.   $$
...
### Theorem 11
  ...
  Poles beginning at complex locations will depart at an angle $$\theta$$ where $$     \sum_{i=1}^m (p + z_i) - \sum_{i=1}^n (p + p_i) = (2l+1)\pi   $$
...
$$\mathbf{x}[k+1] = G\mathbf{x}[k]+H\mathbf{u}[k] \qquad G = e^{AT} \quad H = \int_0^Te^{A\lambda}Bd\lambda.   \qquad (12)$$
If we solve this recursive equation for a particular $$\mathbf{x}[0]$$,
then we get
$$\mathbf{x}[n] = G^n\mathbf{x}[0] + \sum_{i=0}^{n-1}G^iH\mathbf{u}[i].   \qquad (13)$$
...
$$\begin{aligned}   zX(z) - zx[0] = GX(z) + HU(z)\\   X(Z) = (zI-G)^{-1}(HU(z)+x[0]z)   \qquad (14)\end{aligned}$$
...
$$-A^n = \sum_{i=0}^{n-1}c_iA^i.   \qquad (15)$$ This also gives us a
new way to find $$e^{At}$$ because by its Taylor series expansion,
$$e^{At} = \sum_{k=0}^{\infty} \frac{1}{k!}A^k.$$ By equation 14, all
$$A^k = A^{n}A^{k-n}$$ for $$k>n$$ can be expressed in terms of the
lower powers $$A^i$$ for $$i\in[0, n)$$.

{% hint style=\"info\" %}

### Theorem 15

  $$     e^{At} = \sum_{i=0}^{n-1}\alpha_i(t)A^i   $$ for some $$\alpha_i$$ which are solutions to the equations
