...
All points on the root locus must satisfy equation 14.
...
Going back to equation 14, we can alternatively express the angle
criteria as
...
half plane and use equation 15 to find $$Z$$. However, remember that we
...
$$-A^n = \sum_{i=0}^{n-1}c_iA^i.   \qquad (20)$$ This also gives us a
new way to find $$e^{At}$$ because by its Taylor series expansion,
$$e^{At} = \sum_{k=0}^{\infty} \frac{1}{k!}A^k.$$ By equation 19, all
