...
matrix $$D$$. Together, equation 2 and equation 3 are the state-space
...
$$H(s) = \frac{Y(s)}{U(s)} = C(sI-A)^{-1}B+D.   \qquad (4)$$ equation 4
...
Series expansion. With this definition, we can solve equation 2 using
integrating factors. If we let $$e^{-At}$$ be our integrating factor,
then multiplying it to both sides of equation 2 gives
...
$$e^{At} = \sum_{k=0}^{\infty} \frac{1}{k!}A^k.$$ By equation 20, all
