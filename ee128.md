...
::: {.definition}
**Definition 1**. *The plant is the physical system which we would like
to control. [\[defn:plant\]]{#defn:plant label="defn:plant"}*
:::

In general, there are two different types of control.

::: {.definition}
**Definition 2**. *Open-Loop control is where we pass a reference
directly to the actuator to control the plant (see
[\[fig:open-loop\]](#fig:open-loop){reference-type="ref"
reference="fig:open-loop"}). [\[defn:open-loop\]]{#defn:open-loop
...
::: {.definition}
**Definition 3**. *Closed loop control is using the output of our system
and comparing it to the reference in order to generate the control
signal (see [\[fig:closed-loop\]](#fig:closed-loop){reference-type="ref"
reference="fig:closed-loop"}). [\[defn:closed-loop\]]{#defn:closed-loop
...
::: {.definition}
**Definition 4**. *The control law $K(e)$ is a function of error applied
by the controller to determine the inputs to the plant.
[\[defn:control-law\]]{#defn:control-law label="defn:control-law"}*
:::
...
[\[table:electro-mechanical-equivalence\]](#table:electro-mechanical-equivalence){reference-type="ref"
reference="table:electro-mechanical-equivalence"} for their Laplace
domain relationships. At an electrical node, $\sum V = 0$ by Kirchoff's
Voltage Law, and at an electrical junction, $\sum I_{in} = \sum I_{out}$
...
domain) in
[\[table:electro-mechanical-equivalence\]](#table:electro-mechanical-equivalence){reference-type="ref"
reference="table:electro-mechanical-equivalence"}. Under these
analogies, forces are like voltages, currents are like velocities, and
charge is like position.

YYYY Component & Translational Components & Rotational Components &
...
Gears/Transformer & - &
$\frac{T_2(s)}{T_1(s)}=\frac{\Theta_1(s)}{\Theta_2(s)}=\frac{N_2}{N_1}$
& $\frac{N_p}{N_s}=\frac{V_p(s)}{V_s(s)}=\frac{I_s(s)}{I_p(s)}$\
...
::: {.definition}
**Definition 5**. *Linearization is when a nonlinear system $f(\V{x})$
is approximated by the first two terms of its Taylor series about a
particular operating point.
$$f(\V{x}_0 + \delta \V{x}) \approx f(\V{x}_0) + \nabla_x|_{\V{x}_0+\delta\V{x}}\delta\V{x}$$
[\[defn:linearization\]]{#defn:linearization
label="defn:linearization"}*
:::

Using [\[defn:linearization\]](#defn:linearization){reference-type="ref"
reference="defn:linearization"}, we can see that around our operating
point, we have
$$f(\V{x}) - f(\V{x}_0) = \delta f(\V{x}) \approx \nabla_x|_{\V{x}_0+\delta\V{x}} \delta\V{x}
  \label{eqn:linearization}$$
[\[eqn:linearization\]](#eqn:linearization){reference-type="ref"
reference="eqn:linearization"} will hold so long as $\delta\V{x}$ is
small enough to be within the linear regime (i.e where the Taylor Series
expansion is a good approximation). If $f$ is a multi-variable equation,
then [\[eqn:linearization\]](#eqn:linearization){reference-type="ref"
reference="eqn:linearization"} becomes
...
::: {.definition}
**Definition 6**. *System variables are variables which depend on either
the input or the system's internal state.
[\[defn:system-vars\]]{#defn:system-vars label="defn:system-vars"}*
:::

::: {.definition}
**Definition 7**. *The state variables of a system are the smallest set
of linear independent system variables that can uniquely determine all
the other system variables for all $t > 0$.
[\[defn:state-vars\]]{#defn:state-vars label="defn:state-vars"}*
:::

One can think of the state variables $\V{x}$ as capturing the internal
dynamics of the system. The dynamics are described by matrices $A$ (the
state-evolution matrix) and $B$ (the input matrix)
$$\diff[]{\V{x}}{t} = A\V{x} + B\V{u}$$ where $\V{u}$ is the input to
the system. Sometimes the states are not directly observable, but
instead the sensor in
[\[fig:closed-loop\]](#fig:closed-loop){reference-type="ref"
reference="fig:closed-loop"} only provides a linear combination of the
states determined by the output matrix $C$ and the feedforward matrix
$D$. Together,
[\[eqn:state-space-1\]](#eqn:state-space-1){reference-type="ref"
reference="eqn:state-space-1"} and
[\[eqn:state-space-2\]](#eqn:state-space-2){reference-type="ref"
reference="eqn:state-space-2"} are the state-space equations of the
...
sides of
[\[eqn:state-space-1,eqn:state-space-2\]](#eqn:state-space-1,eqn:state-space-2){reference-type="ref"
reference="eqn:state-space-1,eqn:state-space-2"}, $$\begin{aligned}
...
condition is $\V{x}(0^-) = \boldsymbol{0}$, then
$$H(s) = \frac{Y(s)}{U(s)} = C(sI-A)^{-1}B+D.
  \label{eqn:ss-to-tf}$$
[\[eqn:ss-to-tf\]](#eqn:ss-to-tf){reference-type="ref"
reference="eqn:ss-to-tf"} makes it very clear that the poles of the
system are the same as the eigenvalues of the $A$ matrix.
...
It is possible that $\exists M < N$ such that $\forall k \geq M, b_k=0$.
In other words, the numerator can have fewer terms than the denominator.
...
$$\begin{aligned}
  ...
  y(t) &= \frac{1}{a_N}\begin{bmatrix} b_0 - \frac{b_Na_0}{a_N} & b_1 - \frac{b_Na_1}{a_N} & \ldots & b_{N-1} - \frac{b_Na_{N-1}}{a_N} \end{bmatrix} \V{x} + \frac{b_N}{a_N}u(t). \addtocounter{equation}{1}\tag{\theequation}
  \label{eqn:phase-var-2}\end{aligned}$$ Together,
[\[eqn:phase-var-1,eqn:phase-var-2\]](#eqn:phase-var-1,eqn:phase-var-2){reference-type="ref"
reference="eqn:phase-var-1,eqn:phase-var-2"} are known as **Phase
...
control in [6](#sec:ss-control){reference-type="ref"
reference="sec:ss-control"}, this makes it easier to place the system
poles where we want them to be.
...
the same for state-space equations?
[\[eqn:state-space-1\]](#eqn:state-space-1){reference-type="ref"
reference="eqn:state-space-1"} is a inhomogenous, first-order vector
ordinary differential equation. If it was a scalar homogenous ODE, then
we know the solution would be $x(t)=x(0)e^{at}$, so for our vector case,
let us first define $$e^{At} = \sum_{k=0}^{\infty} \frac{1}{k!} A^k$$
using the Taylor Series expansion. With this definition, we can solve
[\[eqn:state-space-1\]](#eqn:state-space-1){reference-type="ref"
reference="eqn:state-space-1"} using integrating factors. If we let
$e^{-At}$ be our integrating factor, then multiplying it to both sides
of [\[eqn:state-space-1\]](#eqn:state-space-1){reference-type="ref"
reference="eqn:state-space-1"} gives
...
both sides from 0 to $t$, $$\begin{aligned}
  ...
  \therefore \V{x}(t) = e^{At}\V{x}(0) + \int_{0}^{t}e^{A(t-\tau)}B\V{u}(\tau)d\tau \addtocounter{equation}{1}\tag{\theequation}\label{eqn:ss-td-sol}\end{aligned}$$
Notice that [\[eqn:ss-td-sol\]](#eqn:ss-td-sol){reference-type="ref"
reference="eqn:ss-td-sol"} is broken into two pieces.

::: {.definition}
**Definition 8**. *The zero-input response is how the system will behave
when no input is supplied. $$\V{x}(t) = e^{At}\V{x}(0)$$
[\[defn:zero-input-response\]]{#defn:zero-input-response
label="defn:zero-input-response"}*
:::

::: {.definition}
**Definition 9**. *The zero-state response is how the system response to
an input when its initial state is $\V{x}(0) = \boldsymbol{0}$. It is
the convolution of the input with $e^{At}B\V{u}(t)u(t)$ where $u(t)$ is
the unit step. $$\V{x}(t) = \int_{0}^{t}e^{A(t-\tau)}B\V{u}(\tau)d\tau$$
[\[defn:zero-state-response\]]{#defn:zero-state-response
...
::: {.definition}
**Definition 10**. *A system is controllable if for any initial state
$\V{x}_0$, we can reach a new state $\V{x}_f$ in finite time with no
constraints on the input $\V{u}$.
[\[defn:controllability\]]{#defn:controllability
label="defn:controllability"}*
:::

Let us assume that we have a controllable system and we want to reach
the state $\V{0}$ from $\V{x}_0$, and we reach it at time $t_f$. Then
using [\[eqn:ss-td-sol\]](#eqn:ss-td-sol){reference-type="ref"
reference="eqn:ss-td-sol"},
$$-\V{x}_0 = \int_0^{t_f} e^{-A\tau}B\V{u}(\tau)d\tau.$$ By the
Cayley-Hamilton Theorem ([8](#appendix:cayley){reference-type="ref"
reference="appendix:cayley"}), $$\begin{aligned}
...
::: {.definition}
**Definition 11**. *The controllability matrix is
$$\mathcal{C} = \begin{bmatrix} B & AB & A^2 B & \ldots & A^{n-1}B \end{bmatrix}.$$
[\[defn:controllability-matrix\]]{#defn:controllability-matrix
label="defn:controllability-matrix"}*
:::

Notice that if $\mathcal{C}$ is invertible, then we can find the $\V{c}$
which will recover $-\V{x}_0$, but if it is not invertible, then we may
not be able to do this.

::: {.theorem}
**Theorem 1**. *If $\mathcal{C}$ is invertible, then the system is
controllable. [\[thm:controllability\]]{#thm:controllability
...
::: {.definition}
**Definition 12**. *A system is observable if for any initial state
$\V{x}_0$, we can determine $\V{x}_0$ from $u(t)$ and $y(t)$ over a
finite time interval. [\[defn:observability\]]{#defn:observability
label="defn:observability"}*
:::

::: {.definition}
**Definition 13**. *The observability matrix is
$$\mathcal{O} = \begin{bmatrix} C \\ CA \\ \\ \vdots \\ CA^{n-1} \end{bmatrix}.$$
[\[defn:observability-matrix\]]{#defn:observability-matrix
label="defn:observability-matrix"}*
:::

A theorem analogous to
[\[thm:controllability\]](#thm:controllability){reference-type="ref"
reference="thm:controllability"} exists for observability.

::: {.theorem}
**Theorem 2**. *If $\mathcal{O}$ is invertible, then the system is
observable. [\[thm:observability\]]{#thm:observability
...
[\[fig:time-delay\]](#fig:time-delay){reference-type="ref"
reference="fig:time-delay"}.

System Performance
==================

::: {.definition}
**Definition 14**. *The step response of a system is how a system $H(s)$
responds to a step input.
$$y(t) = \mathcal{L}^{-1}\left\{ \frac{H(s)}{s} \right\}$$
[\[defn:step-response\]]{#defn:step-response
...
::: {.definition}
**Definition 15**. *A first order system is one with the transfer
function of the form $$H(s) = \frac{s+\alpha}{s+\beta}.$$
[\[defn:first-order\]]{#defn:first-order label="defn:first-order"}*
:::
...
$\beta$ is (i.e the deeper in the left half plane it is), the faster the
system will "settle".
...
::: {.definition}
**Definition 16**. *Second order systems are those with the transfer
function in the form
$$H(s) = \frac{\omega_n^2}{s^2+2\zeta\omega_ns+\omega_n^2}.$$ $\omega_n$
is known as the natural frequency, and $\zeta$ is known as the damping
factor. [\[defn:second-order\]]{#defn:second-order
...
1.  **Undamped**
    ...
    When $\zeta=0$, the poles are $s = \pm \omega_n j$. Because they are
    purely imaginary, the step response will be purely oscillatory.
    $$Y(s) = \frac{1}{s}\frac{\omega_n^2}{s^2+\omega_n^2} \leftrightarrow y(t) = u(t) - \cos(\omega_n t)u(t)$$

2.  **Underdamped**

    When $\zeta\in(0, 1)$, the poles are
    $s = -\zeta\omega_n\pm j\omega_n\sqrt{1-\zeta^2}$. They are complex
    and in the left-half plane, so the step response will be a
    exponentially decaying sinusoid. We define the damped frequency
    $\omega_d = \omega_n\sqrt{1-\zeta^2}$ so that the poles become
    $s=-\zeta\omega_n \pm \omega_dj$. Notice that $\omega_d < \omega_n$.
...
3.  **Critically Damped**
    ...
    When $\zeta=1$, both poles are at $s=-\omega_n$. The poles are both
    real, so the time-response will respond without any overshoot.

4.  **Overdamped**

    When $\zeta>1$, the poles are
    $-\zeta\omega_n\pm \omega_n\sqrt{\zeta^2-1}$. Both of these will be
    real, so the time-response will look similar to a first-order system
    where it is slow and primarily governed by the slowest pole.
...
derivative. $$\begin{aligned}
  ...
  \therefore \diff[]{y}{t} &= \frac{\omega_n^2}{\omega_d}e^{-\zeta\omega_nt}\sin(\omega_d t)u(t) \addtocounter{equation}{1}\tag{\theequation}\label{eqn:2nd-order-deriv}\end{aligned}$$

::: {.definition}
**Definition 17**. *The Time to Peak ($T_p$) of a system is how long it
takes to reach is largest value in the step response.
[\[defn:time-to-peak\]]{#defn:time-to-peak label="defn:time-to-peak"}*
:::

Using
[\[eqn:2nd-order-deriv\]](#eqn:2nd-order-deriv){reference-type="ref"
reference="eqn:2nd-order-deriv"}, we see that the derivative is first
equal to 0 when $t = \frac{\pi}{\omega_d}$.
$$\therefore T_p = \frac{\pi}{\omega_d}$$

::: {.definition}
**Definition 18**. *The Percent Overshoot ($\% O.S$) of a system is by
how much it will overshoot the step response.
[\[defn:percent-os\]]{#defn:percent-os label="defn:percent-os"}*
:::
...
::: {.definition}
**Definition 19**. *The Settling Time ($T_s$) of a system is how long it
takes for the system to start oscillating within 2% of its final value.
[\[defn:settle-time\]]{#defn:settle-time label="defn:settle-time"}*
:::
...
Recall [\[eqn:ss-td-sol\]](#eqn:ss-td-sol){reference-type="ref"
reference="eqn:ss-td-sol"} which told us the time-domain solution to
state-space equations was
$$\V{x}(t) = e^{At}\V{x}(0) + \int_{0}^{t}e^{A(t-\tau)}B\V{u}(\tau)d\tau.$$

::: {.definition}
**Definition 20**. *A system is bounded-input, bounded output(BIBO)
stable if $\exists K_u, K_x < \infty$ such that
$|\V{u}(t)| < K_{u} \implies |\V{x}(t)| < K_x$.
[\[defn:bibo-stability\]]{#defn:bibo-stability
label="defn:bibo-stability"}*
:::

Following from
[\[defn:bibo-stability,eqn:ss-td-sol\]](#defn:bibo-stability,eqn:ss-td-sol){reference-type="ref"
reference="defn:bibo-stability,eqn:ss-td-sol"}, this means that
$$\lim_{t\to\infty}\V{x}(t) = \boldsymbol{0}.$$ If instead
$\lim_{t\to\infty}\V{x}(t) = \infty$, then the system is unstable.

::: {.theorem}
**Theorem 3**. *If all poles are in the left half plane and the number
of zeros is less than or equal to the number of poles, then the system
is BIBO stable. [\[thm:bibo-stability\]]{#thm:bibo-stability
label="thm:bibo-stability"}*
:::

::: {.definition}
**Definition 21**. *A system is called marginally stable if the
zero-input response does not converge to $\boldsymbol{0}$.
[\[defn:marginal-stability\]]{#defn:marginal-stability
label="defn:marginal-stability"}*
:::

::: {.theorem}
**Theorem 4**. *A system is marginally stable if there is exactly one
pole at $s=0$ or a pair of poles at $s=\pm j\omega_0$.
[\[thm:marginal-stability\]]{#thm:marginal-stability
...
[\[fig:unity-feedback-loop\]](#fig:unity-feedback-loop){reference-type="ref"
reference="fig:unity-feedback-loop"} where we put a system $G(s)$ in
unity feedback to control it.

We want to understand what its steady state error will be in response to
different inputs.

::: {.theorem}
**Theorem 5**. *The final value theorem says that for a function whose
unilateral laplace transform has all poles in the left half plane,
$$\lim_{t\to\infty}x(t) = \lim_{s\to0} sX(s).$$
[\[thm:final-value-theorem\]]{#thm:final-value-theorem
...
::: {.definition}
**Definition 22**. *The position constant determines how well a system
can track a unit step. $$K_p = \lim_{s\to0}G(s)
    \label{eqn:position-constant}$$
$$\lim_{t\to\infty} e(t) = \lim_{s\to0} s \frac{1}{s} \frac{1}{1+G(s)} = \frac{1}{1+K_p}$$
[\[defn:position-constant\]]{#defn:position-constant
label="defn:position-constant"}*
:::

::: {.definition}
**Definition 23**. *The velocity constant determines how well a system
can track a ramp. $$K_v = \lim_{s\to0}sG(s)
    \label{eqn:velocity-constant}$$
$$\lim_{t\to\infty} e(t) = \lim_{s\to0} s \frac{1}{s^2} \frac{1}{1+G(s)} = \frac{1}{K_v}$$
[\[defn:velocity-constant\]]{#defn:velocity-constant
label="defn:velocity-constant"}*
:::

::: {.definition}
**Definition 24**. *The acceleration constant determines how well a
system can track a parabola. $$K_a = \lim_{s\to0}s^2G(s)
    \label{eqn:acceleration-constant}$$
$$\lim_{t\to\infty} e(t) = \lim_{s\to0} s \frac{1}{s^3} \frac{1}{1+G(s)} = \frac{1}{K_a}$$
[\[defn:acceleration-constant\]]{#defn:acceleration-constant
label="defn:acceleration-constant"}*
:::

Notice that large static error constants mean a smaller error. Another
observation we can make is that if a system has $n$ poles at $s=0$, it
can perfectly track an input whose laplace transform is
$\frac{1}{s^{n-k}}$ for $k\in[0, n-1]$. We give $n$ a formal name.

::: {.definition}
**Definition 25**. *The system type is the number of poles at 0.
[\[defn:system-type\]]{#defn:system-type label="defn:system-type"}*
:::

This also brings another observation.

::: {.definition}
**Definition 26**. *The internal model principle is that if the system
in the feedback loop has a model of the input we want to track, then it
can track it exactly. [\[defn:interal-model\]]{#defn:interal-model
label="defn:interal-model"}*
:::

If instead we have a state-space system, then assuming the system is
stable,
$$\lim_{t\to\infty}\diff[]{\V{x}}{t} = \boldsymbol{0} \implies \lim_{t\to\infty}\V{x} = \V{x}_{ss}.$$
Applying this to the state space equations for a step input,
$$\diff[]{\V{x}}{t} = \boldsymbol{0} = A\V{x}_{ss} + B\cdot I \implies \V{x}_{ss} = -A^{-1}B
...
::: {.definition}
**Definition 27**. *The frequency response of the system determines how
it scales pure frequencies. It is equivalent to the Laplace transform
evaluated on the imaginary axis.
$$G(j\omega) = \int_0^{\infty}g(\tau)e^{-j\omega\tau}d\tau
    \label{eqn:frequency-response}$$
[\[defn:frequency-response\]]{#defn:frequency-response
label="defn:frequency-response"}*
:::

Suppose we put a linear system $G(s)$ in negative feedback. We know that
if $\angle G(j\omega) = (2k+1)\pi$ for some $k\in\mathbb{Z}$, then the
output of the plant will be $-|G(j\omega)|e^{j\omega t}$. If
$|G(j\omega)| \geq 1$, then this will feed back into the error term
where it will be multiplied by $|G(j\omega)|$ repeatedly, and this will
cause the system to be unstable because $|G(j\omega)|\geq1$ and thus
will not decay.

::: {.definition}
**Definition 28**. *The gain margin $G_m$ is the change in the open loop
gain required to make the closed loop system unstable.
[\[defn:gain-margin\]]{#defn:gain-margin label="defn:gain-margin"}*
:::

::: {.definition}
**Definition 29**. *The phase margin $\phi_m$ is the change in the open
loop phase required to make the closed loop system unstable.
[\[defn:phase-margin\]]{#defn:phase-margin label="defn:phase-margin"}*
:::

We can imagine the gain and phase margin like placing a "virtual box"
before the plant as shown in
[\[fig:margin-controller\]](#fig:margin-controller){reference-type="ref"
reference="fig:margin-controller"}.
...
where the gain margin frequency is $\angle G(j\omega_m) = (2k+1)\pi$ for
$k\in\mathbb{Z}$. Likewise, at the phase margin frequency $\omega_{pm}$,
...
[\[fig:rl-feedback-controller\]](#fig:rl-feedback-controller){reference-type="ref"
reference="fig:rl-feedback-controller"}. It would be helpful for us to
understand how the closed loop poles of the feedback system change as
...
::: {.definition}
**Definition 30**. *The root locus is the set of all $s\in\mathbb{C}$
such that $\exists K$ where $\Delta(s) = 0$.
[\[defn:root-locus\]]{#defn:root-locus label="defn:root-locus"}*
:::

All points on the root locus must satisfy
[\[eqn:rl-criteria\]](#eqn:rl-criteria){reference-type="ref"
reference="eqn:rl-criteria"}.

### Root Locus Rules

First, notice that the roots of $\Delta(s)$ are the closed loop poles of
the system.

::: {.theorem}
**Theorem 6**. *The number of branches in the root locus is equal to the
number of closed loop poles where a branch is the path traveled by a
single pole as $K$ is varied. [\[thm:rl-one\]]{#thm:rl-one
label="thm:rl-one"}*
:::
...
::: {.theorem}
**Theorem 7**. *The root locus is symmetric about the real axis.
[\[thm:rl-two\]]{#thm:rl-two label="thm:rl-two"}*
:::

Going back to
[\[eqn:rl-criteria\]](#eqn:rl-criteria){reference-type="ref"
reference="eqn:rl-criteria"}, we can alternatively express the angle
criteria as
...
restrict ourselves to the real axis, then given a closed loop pole $s$,
each $z_i > s$ will contribute $-180$and each $p_i > s$ will contribute
$180$while the $z_i, p_i < s$ will contribute $0$.

::: {.theorem}
**Theorem 8**. *The real axis segments of the root locus are to the left
of an odd number of open loop poles and zeros.
[\[thm:rl-three\]]{#thm:rl-three label="thm:rl-three"}*
:::

When $K$ is small, then the poles look like the open loop poles. As $K$
grows very large, then the poles look like the open loop zeros.

::: {.theorem}
**Theorem 9**. *The root locus begins at the open loop poles and ends at
the open loop zeros [\[eqn:rl-four\]]{#eqn:rl-four label="eqn:rl-four"}*
:::
...
::: {.theorem}
**Theorem 10**. *In the limit, poles will asymptotically approach
$$\theta = \frac{-(2l+1)\pi}{n-m},$$ and the real axis intercept of
these asymptotes is
$$\sigma = \frac{\sum_{i=1}^m p_i - \sum_{i=1}^n z_i}{n-m}.$$
[\[eqn:rl-five\]]{#eqn:rl-five label="eqn:rl-five"}*
:::
...
::: {.theorem}
**Theorem 11**. *The break-in and break-away points satisfy the equation
$$\sum_{i=1}^n \frac{1}{\sigma + p_i} = \sum_{i=1}^m \frac{1}{\sigma+z_i}.$$
[\[thm:rl-six\]]{#thm:rl-six label="thm:rl-six"}*
:::
...
::: {.theorem}
**Theorem 12**. *The root locus intersects the imaginary axis at points
where
$$\sum_{i=1}^m \angle (j\omega + z_i) - \sum_{i=1}^n \angle (j\omega + p_i) = (2l+1)\pi.$$
[\[thm:rl-seven\]]{#thm:rl-seven label="thm:rl-seven"}*
:::
...
::: {.theorem}
**Theorem 13**. *Poles beginning at complex locations will depart at an
angle $\theta$ where
$$\sum_{i=1}^m (p + z_i) - \sum_{i=1}^n (p + p_i) = (2l+1)\pi$$*

*[\[thm:rl-eight\]]{#thm:rl-eight label="thm:rl-eight"}*
:::
...
::: {.theorem}
**Theorem 14**. *Given a pole location $p$,
$$K = \left|\frac{1}{G(p)H(p)}\right|.$$ [\[thm:rl-nine\]]{#thm:rl-nine
label="thm:rl-nine"}*
:::
...
::: {.definition}
**Definition 31**. *A Bode plot is a plot of the magnitude and phase of
the frequency response with the magnitude on a log-log scale and the
phase on a semi-log scale. [\[defn:bode-plot\]]{#defn:bode-plot
...
where $\omega_n$ is either a zero or a pole, $z_i, p_k$ are the phases
of each factor, and $N_{z0}, N_{p0}$ are the number of zeros and poles
at 0. By writing $G(\omega)$ this way, it is clear that
...
Each $p_k$ and $z_i$ are of the form $1 + \frac{j\omega}{\omega_n}$. If
$\omega > 10\omega_n$, then $p_k, z_i \approx \omega_n$. Likewise, if
$\omega < \frac{\omega_n}{10}$, $p_k, z_i \approx 1$. This means we can
approximate bode plots using piece-wise linear segments using the
following rules.
...
3.  Each zero $\omega_z$ contributes
    $\SI[per-mode=symbol]{45}{\degree\per\decade}$ to phase starting at
    $\frac{\omega_z}{10}$ and ending at $10\omega_z$.

4.  Each pole $\omega_p$ contributes
    $\SI[per-mode=symbol]{-45}{\degree\per\decade}$ to phase starting at
    $\frac{\omega_p}{10}$ and ending at $10\omega_p$.
...
[\[fig:nyquist-feedback-controller\]](#fig:nyquist-feedback-controller){reference-type="ref"
reference="fig:nyquist-feedback-controller"} and suppose that
$$G(s) = \frac{N_G}{D_G} \qquad H(s) = \frac{N_H}{D_H}.$$ Then the
...
here, we can see that the poles of $1+GH$ are the poles of the open loop
system whereas the zeros of $1+GH$ are the poles of the closed loop
system.

::: {.definition}
**Definition 32**. *A contour is a closed loop set of points in the
complex plane. [\[defn:contour\]]{#defn:contour label="defn:contour"}*
:::

::: {.definition}
**Definition 33**. *A mapping is a function that takes a point in the
complex plane and transforms it into another point in the complex plane.
[\[defn:mapping\]]{#defn:mapping label="defn:mapping"}*
:::

::: {.definition}
**Definition 34**. *The Nyquist Criterion says that if $N$ is the number
of counter-clockwise encirclements of zero of a contour mapped by a
transfer function $F(s)$, $P$ is the number of poles in the contour, and
$Z$ is the number of zeros in the contour, then $$N = P - Z.
    \label{eqn:nyquist-criteria}$$
[\[defn:nyquist-criteria\]]{#defn:nyquist-criteria
...
half plane and use
[\[eqn:nyquist-criteria\]](#eqn:nyquist-criteria){reference-type="ref"
reference="eqn:nyquist-criteria"} to find $Z$. However, remember that we
...
encirclements of $-1$. Once we have $Z$, we know how many RHP poles the
closed loop transfer function will have because they are the same as the
RHP zeros of the $1+GH$. We can extend the Nyquist Criterion to finding
a range of gains that would make the open-loop system $kG(s)H(s)$ stable
by looking for the encirclements of $\frac{-1}{K}$.
...
plant like in
[\[fig:cascade-comp\]](#fig:cascade-comp){reference-type="ref"
reference="fig:cascade-comp"}.
...
::: {.definition}
**Definition 35**. *A proportional integral controller applies an input
which is a linear combination of the scaled and integrated error signal.
$$H(s) = K_p+\frac{K_i}{s} = K_p \frac{s + \frac{K_i}{K_p}}{s}$$
[\[defn:pi-controller\]]{#defn:pi-controller
label="defn:pi-controller"}*
:::

What a PI Controller effectively does is place a new pole at $s=0$ and a
new zero at $s=-\frac{K_i}{K_p}$. Because of the pole at zero, adding a
PI controller enables the system to track a step input perfectly.
However, because the pole is placed at zero, it can slow down the time
response unless the zero can cancel it out.

Proportional Derivative (PD) Control
------------------------------------

::: {.definition}
**Definition 36**. *A proportional derivative controller applies an
input which is a linear combination of the scaled and differentiated
error signal.
$$H(s) = K_p + K_ds = K_d\left(s + \frac{K_p}{K_d}\right)$$
[\[defn:pd-controller\]]{#defn:pd-controller
...
the location of the zero $s=-\frac{K_p}{K_d}$. Since differentiation is
an unstable operations, sometimes we instead also place a pole very far
in the left half plane, and the transfer function becomes
...
::: {.definition}
**Definition 37**. *A proportional integral derivative controller
applies an input which is a linear combination of the scaled,
differentated, and integrated error signals.
$$H(s) = K_p+K_ds + \frac{K_i}{s} = K_d \frac{s^2 + \frac{K_p}{K_d}s + \frac{K_i}{K_d}}{s}$$
[\[defn:pid-controller\]]{#defn:pid-controller
...
::: {.definition}
**Definition 38**. *A lag compensator is a controller with the transfer
function $$H(s) = K \frac{s+z_c}{s+p_c},\quad p_c < z_c.$$
[\[defn:lag-network\]]{#defn:lag-network label="defn:lag-network"}*
:::
...
plot in [\[fig:lag-network\]](#fig:lag-network){reference-type="ref"
reference="fig:lag-network"}).
...
2.  Find $\omega_{PM}$ such that $\phi_M$ is 5to 12larger than required.
...
::: {.definition}
**Definition 39**. *A Lead Controller is a compensator with the transfer
function
$$H(s) = k\frac{s+z_c}{s+p_c} = \frac{k}{\beta}\frac{s+\frac{1}{T}}{s+\frac{1}{\beta T}} \quad z_c > p_c, \beta < 1.$$
[\[defn:lead-controller\]]{#defn:lead-controller
...
Its frequency response looks like in
[\[fig:lead-network\]](#fig:lead-network){reference-type="ref"
reference="fig:lead-network"}.
...
3.  Find $\phi_{M}$ needed to meet requirements and evaluate additional
    phase contribution from compenstor.
...
our poles where we want since the last row of the $A$ matrix is also the
...
can't use $\V{u}=\V{r}-K\V{x}$ because we don't know $\V{x}$. One idea
is to keep track of an estimated state $\hat{\V{x}}$ and estimated
output $\hat{\V{y}}$ which follow the same system dynamics as the actual
state and the actual output and receive the same input. If $A$ is a
stable matrix, then $\lim_{t\to\infty}e^{At}\V{x}_0=0$ where $\V{x}_0$
is the initial state of the system. This means that even if there is a
...
the estimated state $\V{e} = \V{x} - \hat{\V{x}}$, so we add a gain $L$
to the error in the outputs $\V{y}-\hat{\V{y}}$. $$\begin{aligned}
...
design $L$ to get quick error convergence. Notice that if our system is
not observable, then we will not be able to place the poles of the
...
$$\begin{bmatrix} \diff[]{\V{x}}{t} \\ \diff[]{\V{e}}{t} \end{bmatrix}
  ...
  + \begin{bmatrix} B \\ \boldsymbol{0} \end{bmatrix}\V{r}.$$ Notice
that the poles of this system are just the poles of the original system
and the poles of the observer system, so we can choose $K$ and $L$
independently.
...
$$\begin{aligned}
  ...
  + \begin{bmatrix} B \\ \boldsymbol{0} \end{bmatrix} + \begin{bmatrix} \boldsymbol{0} \\ \boldsymbol{I} \end{bmatrix}\V{r}.\end{aligned}$$
When we apply our feedback rule, we get $$\begin{aligned}
  ...
  + \begin{bmatrix} \boldsymbol{0} \\ \boldsymbol{I} \end{bmatrix}\V{r}.\end{aligned}$$
...
$-K\V{x}$. We want to do this by optimizing a cost function that
penalizes control effort and the state error. In particular, we want to
minimize the cost function
$$J = \int_{0}^{\infty}\V{y}^TQ\V{y} + \V{u}^TR\V{u} dt.$$ where $R$ and
$Q$ are positve-semi-definite matrices (typically diagonal) and
...
If we look only at the output at times which are multiples of $kT$ (i.e
what our CPU would see if we sampled the output), then we can find a
relationship between the current sample and the next sample.
$$\begin{aligned}
...
depends on the eigenvales of $G$ and that $|\lambda_G|<1$ in order for
it to be stable. If we want to find a transfer function of the system,
then we can use the unilateral Z-transform. $$\begin{aligned}
  zX(z) - zx[0] = GX(z) + HU(z)\\
  X(Z) = (zI-G)^{-1}(HU(z)+x[0]z) \addtocounter{equation}{1}\tag{\theequation}\label{eqn:ss-dt-tf}\end{aligned}$$
Notice that the poles of the system are still the eigenvalues of the $G$
matrix. Thus any techniques for placing the poles of the $G$ matrix are
still valid, except we need to make sure the poles stay within the unit
circle for stability instead of the left half plane. If we want to
figure out how a system will respond to an input, we can use the final
value theorem like we do in CT.

::: {.theorem}
**Theorem 15**. *For a transfer function $G(z)$ which has all poles in
the unit circle, the final value of $g[n]$ is given by
$$\lim_{n=\to\infty}g[n] = \lim_{z\to1}(z-1)G(z).$$
[\[thm:dt-final-val\]]{#thm:dt-final-val label="thm:dt-final-val"}*
:::
...
::: {.theorem}
**Theorem 16**. *Every square matrix $A$ satisfies its own
characteristic polynomial if there are no repeated eigenvalues.
$$\Delta(A) = 0$$ [\[thm:cayley-hamilton\]]{#thm:cayley-hamilton
...
$\Lambda^n + \sum_{i=0}^{n-1}c_i \Lambda^i$ is itself a diagonal matrix
where the jth entry on the diagonal is
$$\lambda_j^n + \sum_{i=0}^{n-1}c_i\lambda_j = 0$$ since $\lambda_j$ is
a root of the characteristic polynomial. Thus
...
[\[eqn:an-exponential\]](#eqn:an-exponential){reference-type="ref"
reference="eqn:an-exponential"}, all $A^k = A^{n}A^{k-n}$ for $k>n$ can
be expressed in terms of the lower powers $A^i$ for $i\in[0, n)$.

::: {.theorem}
**Theorem 17**. *$$e^{At} = \sum_{i=0}^{n-1}\alpha_i(t)A^i$$ for some
$\alpha_i$ which are solutions to the equations
$$e^{\lambda_jt} = \sum_{i=0}^{n-1}\alpha_i(t)\lambda_j^i.$$
[\[thm:matrix-exponential\]]{#thm:matrix-exponential
