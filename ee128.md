...
### Definition 5
  ...
  Linearization is when a nonlinear system $$f(\bf{x})$$

{% endhint %}

Using `\cref{defn:linearization}`{=latex}, we can see that around our
operating point, we have
$$f(\bf{x}) - f(\bf{x}_0) = \delta f(\bf{x}) \approx \nabla_x|_{\bf{x}_0+\delta\bf{x}} \delta\bf{x}
  \label{eqn:linearization}$$ `\cref{eqn:linearization}`{=latex} will
hold so long as $$\delta\bf{x}$$ is small enough to be within the linear
...
$$\delta f(\bf{x}, \bf{u}, \dots) \approx \nabla_x|_{\bf{x}_0+\delta\bf{x}} \bf{\delta x} + \nabla_u|_{\bf{u}_0+\delta\bf{u}} \bf{\delta u} + \cdots$$
...
One can think of the state variables $$\bf{x}$$ as capturing the
...
$$\frac{d^{}\bf{x}}{dt^{}} = A\bf{x} + B\bf{u}$$ where $$\bf{u}$$ is the
input to the system. Sometimes the states are not directly observable,
but instead the sensor in `\cref{fig:closed-loop}`{=latex} only provides
a linear combination of the states determined by the output matrix $$C$$
...
system. $$\begin{aligned}
  \frac{d^{}\bf{x}}{dt^{}} &= A\bf{x} + B\bf{u} \label{eqn:state-space-1}\\
  \bf{y} &= C\bf{x} + D \bf{u} \label{eqn:state-space-2}\end{aligned}$$
...
$$\begin{aligned}
  s\bf{X}(s) - \bf{x}(0^-) &= A\bf{X}(s) + B\bf{U}(s)\\
  &\implies \bf{X}(s) = (sI-A)^{-1}B\bf{U}(s) + \bf{x}(0^{-})\\
  \bf{Y}(s) &= C\bf{X}(s) + D\bf{U}(s)\\
  &\implies \bf{Y}(s) = (C\left( sI-A \right)^{-1}B+D)\bf{U}(s) + C(sI-A)^{-1}\bf{x}(0^-).\end{aligned}$$
If the system is Single-Input, Single-Output (SISO) and the initial
condition is $$\bf{x}(0^-) = \boldsymbol{0}$$, then
...
$$\begin{aligned}
  ...
  y(t) &= \frac{1}{a_N}\begin{bmatrix} b_0 - \frac{b_Na_0}{a_N} & b_1 - \frac{b_Na_1}{a_N} & \ldots & b_{N-1} - \frac{b_Na_{N-1}}{a_N} \end{bmatrix} \bf{x} + \frac{b_N}{a_N}u(t). \eqnnumber
...
$$e^{-At}\frac{d^{}\bf{x}}{dt^{}} = e^{-At}A\bf{x} + e^{-At}B\bf{u}.$$
Notice that
$$\frac{d}{dt}\left[ e^{-At}\bf{x} \right] = e^{-At}\frac{d^{}\bf{x}}{dt^{}} - A e^{-At}\bf{x}.$$
Combining these two equations, we see that
$$\frac{d}{dt}\left[ e^{-At}\bf{x} \right] = e^{-At}B\bf{u}.$$
Integrating both sides from 0 to $$t$$, $$\begin{aligned}
  e^{-At}\bf{x}(t) - \bf{x}(0) = \int_{0}^{t}e^{-A\tau}B\bf{u}(\tau)d\tau\\
  \therefore \bf{x}(t) = e^{At}\bf{x}(0) + \int_{0}^{t}e^{A(t-\tau)}B\bf{u}(\tau)d\tau \eqnnumber \label{eqn:ss-td-sol}\end{aligned}$$
...
### Definition 9
  ...
  The zero-state response is how the system response to an input when its initial state is $$\bf{x}(0) = \boldsymbol{0}$$. It is the convolution of the input with $$e^{At}B\bf{u}(t)u(t)$$ where $$u(t)$$
...
### Definition 10
  ...
  A system is controllable if for any initial state $$\bf{x}_0$$, we can reach a new state $$\bf{x}_f$$ in finite time with no constraints on the input $$\bf{u}$$

{% endhint %}

Let us assume that we have a controllable system and we want to reach
the state $$\bf{0}$$ from $$\bf{x}_0$$, and we reach it at time $$t_f$$.
Then using `\cref{eqn:ss-td-sol}`{=latex},
$$-\bf{x}_0 = \int_0^{t_f} e^{-A\tau}B\bf{u}(\tau)d\tau.$$ By the
Cayley-Hamilton Theorem (`\cref{appendix:cayley}`{=latex}),
$$\begin{aligned}
  -\bf{x}_0 = \sum_{j=0}^{n-1}A^jB\int_0^{t_f}\alpha_j(\tau)\bf{u}(\tau)d\tau\\
...
$$\bf{c}$$ which will recover $$-\bf{x}_0$$, but if it is not
...
### Definition 12
  ...
  A system is observable if for any initial state $$\bf{x}_0$$, we can determine $$\bf{x}_0$$ from $$u(t)$$ and $$y(t)$$
...
### Definition 14
  ...
  The step response of a system is how a system $$H(s)$$
...
$$
  ...
  $$\omega_n$$ is known as the natural frequency, and $$\zeta$$
...
$$\bf{x}(t) = e^{At}\bf{x}(0) + \int_{0}^{t}e^{A(t-\tau)}B\bf{u}(\tau)d\tau.$$

{% hint style=\"info\" %}

### Definition 20

  A system is bounded-input, bounded output(BIBO) stable if $$\exists K_u, K_x < \infty$$ such that $$|\bf{u}(t)| < K_{u} \implies |\bf{x}(t)| < K_x$$

{% endhint %}

Following from `\cref{defn:bibo-stability,eqn:ss-td-sol}`{=latex}, this
means that $$\lim_{t\to\infty}\bf{x}(t) = \boldsymbol{0}.$$ If instead
$$\lim_{t\to\infty}\bf{x}(t) = \infty$$, then the system is unstable.
...
stable,
$$\lim_{t\to\infty}\frac{d^{}\bf{x}}{dt^{}} = \boldsymbol{0} \implies \lim_{t\to\infty}\bf{x} = \bf{x}_{ss}.$$
Applying this to the state space equations for a step input,
$$\frac{d^{}\bf{x}}{dt^{}} = \boldsymbol{0} = A\bf{x}_{ss} + B\cdot I \implies \bf{x}_{ss} = -A^{-1}B
  \label{eqn:ss-sse}$$ Looking at the error between the reference and
the output in the 1D input case,
$$\bf{e}(t) = \bf{r}(t) - \bf{y}(t) = 1 - C\bf{x}_{ss} = 1 + CA^{-1}B.$$
...
### Theorem 6
  ...
  The number of branches in the root locus is equal to the number of closed loop poles where a branch is the path traveled by a single pole as $$K$$
...
### Definition 34
  ...
  The Nyquist Criterion says that if $$N$$ is the number of counter-clockwise encirclements of zero of a contour mapped by a transfer function $$F(s)$$, $$P$$ is the number of poles in the contour, and $$Z$$
...
![](.gitbook/assets/fddff32e212e6c6a35ccc06c7b99293e08386e01.png)
...
\centering
```
![](.gitbook/assets/598e0f6c6d06ed829d9919eab6d05494394e6ea3.png)

![](.gitbook/assets/afdcee4d43345bb084ae06c4606c56c58d7c794c.png)
...
$$\frac{d^{}\bf{x}}{dt^{}} = A\bf{x}+B\bf{u},$$ then we can let
$$\bf{u} = \bf{r} - K\bf{x}$$. If we do this, then the equivalent
state-evolution equation becomes
$$\frac{d^{}\bf{x}}{dt^{}} = (A-BK)\bf{x}+B\bf{r}.$$ Notice that if our
system is in phase variable form, then the controlled state-evolution
equation is $$\frac{d^{}\bf{x}}{dt^{}} = \begin{bmatrix}
    0 & 1 & 0 & 0 & \cdots\\
    0 & 0 & 1 & 0 & \cdots\\
    0 & 0 & \ddots & \ddots & \ddots\\
    0 & 0 & \cdots & 0 & 1\\
    -a_0-k_0 & -a_1-k_1 & \cdots & -a_{n-2}-k_{n-2} & -a_{n-1}k_{n-1}
  \end{bmatrix}\bf{x} + B\bf{r}.$$ This makes it very convenient to
place our poles where we want since the last row of the $$A$$ matrix is
also the coefficients of the characteristic polynomial.

Design by Transformation
------------------------

Suppose we have a system
$$\frac{d^{}\bf{z}}{dt^{}} = A\bf{z}+B\bf{u} \qquad \bf{y} = C\bf{z}$$
which is not in phase variable form. To place it into phase variable
form, first assume that $$\bf{z} = P\bf{x}$$ for some invertible matrix
$$P$$.
$$P\frac{d^{}\bf{x}}{dt^{}} = AP\bf{x}+B\bf{u} \implies \frac{d^{}\bf{x}}{dt^{}} + P^{-1}B\bf{u} \qquad \bf{y} = CP\bf{x}.$$
...
$$\frac{d^{}\bf{x}}{dt^{}} = P^{-1}AP\bf{x}+P^{-1}B(-K\bf{x}+\bf{r}) = P^{-1}(AP-BK)P^{-1}\mathbb z + Br \implies K_z = K_zP^{-1}.$$

Observers/State Estimators
--------------------------

When doing state feedback, we often don't have access to the states
themselves because we only have access to $$\bf{y}$$. In that case, we
can't use $$\bf{u}=\bf{r}-K\bf{x}$$ because we don't know $$\bf{x}$$.
One idea is to keep track of an estimated state $$\hat{\bf{x}}$$ and
estimated output $$\hat{\bf{y}}$$ which follow the same system dynamics
as the actual state and the actual output and receive the same input. If
$$A$$ is a stable matrix, then $$\lim_{t\to\infty}e^{At}\bf{x}_0=0$$
where $$\bf{x}_0$$ is the initial state of the system. This means that
...
the estimated state $$\bf{e} = \bf{x} - \hat{\bf{x}}$$, so we add a gain
$$L$$ to the error in the outputs $$\bf{y}-\hat{\bf{y}}$$.
$$\begin{aligned}
  \frac{d^{}\hat{\bf{x}}}{dt^{}} &= A\hat{\bf{x}} + B\bf{u} + L(\bf{y}-\hat{\bf{y}}) = A\hat{\bf{x}}+B\bf{u}+LC(\bf{x}-\hat{\bf{x}})\\
  \frac{d^{}\bf{e}}{dt^{}} &= \frac{d^{}(\bf{x}-\hat{\bf{x}})}{dt^{}} = A\bf{x}+B\bf{u} - \left[A\hat{\bf{x}}+B\bf{u}+LC(\bf{x}-\hat{\bf{x}})\right]\\
  \therefore \frac{d^{}\bf{e}}{dt^{}} &= (A-LC)\bf{e}\end{aligned}$$
...
$$\frac{d^{}\bf{x}}{dt^{}} = A\bf{x}+B(\bf{r}-K\hat{\bf{x}}) = (A-BK)\bf{x}+B(\bf{r}-K\bf{e}).$$
Looking at the combined system,
$$\begin{bmatrix} \frac{d^{}\bf{x}}{dt^{}} \\ \frac{d^{}\bf{e}}{dt^{}} \end{bmatrix}
  = \begin{bmatrix} A-BK & -BK \\ 0 & A-LC \end{bmatrix} \begin{bmatrix} \bf{x} \\ \bf{e} \end{bmatrix}
  + \begin{bmatrix} B \\ \boldsymbol{0} \end{bmatrix}\bf{r}.$$ Notice
...
\centering 
```
![](.gitbook/assets/5ed6af6d5b380962e0195a63b75726d3472efeaf.png)
...
observed outputs. $$\bf{x}_N = \int_{0}^{t}(\vec{r}-\vec{y})dt.$$ If we
treat this as a new state, its evolution will be
$$\frac{d^{}\bf{x}_N}{dt^{}} = r - C\bf{x}.$$ If our new control law is
$$\bf{u} = -K\bf{x}+K_e\bf{x}_N$$, then our new state-space equations
are $$\begin{aligned}
  \begin{bmatrix} \frac{d^{}\bf{x}}{dt^{}} \\ \frac{d^{}\bf{x}_N}{dt^{}} \end{bmatrix}
  = \begin{bmatrix} A & 0 \\ -C & 0 \end{bmatrix} \begin{bmatrix} \bf{x} \\ \bf{x}_N \end{bmatrix}
  + \begin{bmatrix} B \\ \boldsymbol{0} \end{bmatrix} + \begin{bmatrix} \boldsymbol{0} \\ \boldsymbol{I} \end{bmatrix}\bf{r}.\end{aligned}$$
When we apply our feedback rule, we get $$\begin{aligned}
  \begin{bmatrix} \frac{d^{}\bf{x}}{dt^{}} \\ \frac{d^{}\bf{x}_N}{dt^{}} \end{bmatrix}
  = \begin{bmatrix} A-BK & BK_e \\ -C & 0 \end{bmatrix} \begin{bmatrix} \bf{x} \\ \bf{x}_N \end{bmatrix}
  + \begin{bmatrix} \boldsymbol{0} \\ \boldsymbol{I} \end{bmatrix}\bf{r}.\end{aligned}$$
...
$$-K\bf{x}$$. We want to do this by optimizing a cost function that
penalizes control effort and the state error. In particular, we want to
minimize the cost function
$$J = \int_{0}^{\infty}\bf{y}^TQ\bf{y} + \bf{u}^TR\bf{u} dt.$$ where
$$R$$ and $$Q$$ are positve-semi-definite matrices (typically diagonal)
and determine how much we penalize the error and the control effort.
...
$$\bf{x}(kT) = e^{AkT}\bf{x}(0) + \int_{0}^{kT}e^{A(kT-\tau)}B\bf{u}(\tau)d\tau.$$
...
$$\begin{aligned}
  -e^{At}\bf{x}(kT) + \bf{x}((k+1)T) &= -e^{A(k+1)\tau}\bf{x}(0) - \int_{0}^{kT} e^{A((k+1)T-\tau)}B\bf{u}(\tau) d\tau \\
  &+ e^{A(k+1)\tau}\bf{x}(0) + \int_{0}^{(k+1)T} e^{A((k+1)T-\tau)}B\bf{u}(\tau) d\tau\\
  &= \int_{kT}^{(k+1)T} e^{A((k+1)T-\tau)}B\bf{u}(\tau)d\tau = e^{A(k+1)T}B\bf{u}(kT)\int_{kT}^{(k+1)T}e^{-A\tau}d\tau\\
  &= B\bf{u}(kT)\int_0^{T}e^{A\lambda}d\lambda \qquad (\lambda = (k+1)T-\tau)\\
  \therefore \bf{x}((k+1)T) &= e^{At}\bf{x}(kT) + \bf{u}(kT)\int_{0}^{T}Be^{A\lambda}d\lambda.\end{aligned}$$
Looking only at the samples, the discrete system is
$$\bf{x}[k+1] = G\bf{x}[k]+H\bf{u}[k] \qquad G = e^{AT} \quad H = \int_0^Te^{A\lambda}Bd\lambda.
  \label{eqn:ss-discrete}$$ If we solve this recursive equation for a
particular $$\bf{x}[0]$$, then we get
$$\bf{x}[n] = G^n\bf{x}[0] + \sum_{i=0}^{n-1}G^iH\bf{u}[i].
...
### Theorem 15
  ...
  For a transfer function $$G(z)$$ which has all poles in the unit circle, the final value of $$g[n]$$
