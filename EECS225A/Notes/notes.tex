...
\]
Note that $S_X^-(z) = \left(S_X^+(z)\right)^*$.
...
% TODO: Why do we use the same coefficients for each $X(n)$? Do the proof, or see the proof
...
From the spectral factorization of $Y$, we know if we use the filter $G(z) =\frac{1}{S_Y^+(z)}$ then \[
	S_W(z) = \frac{S_Y(z)}{|S_Y^+(z)|^2} = \frac{S_Y(z)}{S_Y^+(z)S_Y^-(z)} = 1.
\]
