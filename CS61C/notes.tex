...
\begin{figure}[H]
    \begin{tikzpicture}[node distance=3cm]
        \node (processor) [box] {Processor};
        \node (tlb) [box, right of=processor] {TLB};
        \node (cache) [box, right of=tlb] {Cache};
        \node (memory) [box, right of=cache, xshift=2cm] {Memory};
        \node (pt) [box, below of=tlb, yshift=1cm] {Page Table};
        \node (connection) [below of=pt, yshift=1cm] {};

        \draw[arrow] (processor) -- node[anchor=south] {VA} (tlb);
        \draw[arrow] (tlb) -- node[anchor=south] {PA} (cache);
        \draw[arrow] (cache) -- node[anchor=south] {On Cache Miss} (memory);
        \draw[arrow] (tlb.south) ++ (-0.5, 0) -- node[anchor=east] {VPN} ($(pt.north)-(0.5,0)$);
        \draw[arrow] (pt.north) ++ (0.5, 0) -- node[anchor=west] {PPN} ($(tlb.south) + (0.5, 0)$);
        \draw[arrow] (cache) |- (connection);
        \draw[arrow] (connection) ++ (2, 0) -| (processor);
        \draw[arrow] (memory) |- (connection);
    \end{tikzpicture}
\end{figure}
