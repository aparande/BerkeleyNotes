...
Since $$\tilde{Y}_{n}$$ is orthogonal to
$$\mathbb{L}\left[Y_n|\tilde{Y}^{(n-1)}\right]$$, they belong to
different parts of the subspace formed by $$Y_1,\cdots,Y_n$$.
...
$$X$$ and $$Y$$ are Jointly Gaussian, then the LLSE does, in fact, equal
the MMSE.
...
Suppose we want to linearly estimate $$\boldsymbol{X}_n$$ from the
$$\boldsymbol{Y}_n$$ we have seen so far.
